...
public class NGramMap {
    ...
    public TimeSeries<Integer> countHistory(String word, int startYear,
        ...
        for (int year = startYear; year <= endYear; year++) {
            if (map.containsKey(year) && map.get(year).count(word) > 0) {
    ...
    public TimeSeries<Double> weightHistory(String word, int startYear,
        ...
        for (int year = startYear; year <= endYear; year++) {
            if (timeMap.containsKey((long) year) && count.containsKey(year)) {
                time.put(year, timeMap.get((long) year).doubleValue());
            }
        }
    ...
    public TimeSeries<Double> weightHistory(String word) {
        ...
        TimeSeries<Double> time = new TimeSeries<Double>();
        for (long year : timeMap.keySet()) {
            if (timeMap.containsKey(year) && count.containsKey((int) year)) {
